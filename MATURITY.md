...
### 🟡 **Library Ecosystem Real, But Uneven**

**Working / substantially implemented libraries:**
- `ilib/math` — Complete
- `ilib/string-cheese` — Complete for common string work
- `ilib/os` — File I/O, process helpers, environment, cwd
- `ilib/regex` — Match/search/replace/split/groups
- `ilib/gl` — SDL2-backed 2D graphics
- `ilib/widgets` — GTK3-backed widgets
- `ilib/machine-audio` — TTS/playback/recording with graceful degradation
- `ilib/pointers` — Runtime pointer registry and pointer helpers
- `ilib/web` — Browser helpers and page fetch; wired for compiled backends
- `ilib/camera` — Real C++/FFI scaffolding, still needs stronger coverage

**Still weak or missing:**
- `ilib/ml` — Spec/FFI only; no real C++ core yet
- JSON/parsing, crypto/hashing, database drivers
- Full web-server stack: listen/route/request/response is not implemented yet
