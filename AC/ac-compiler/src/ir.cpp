...
class IRGenerator {
            ...
            case NodeType::BinaryExpr: {
                ...
                if (expr.children.size() >= 2) {
                    ...
                    if (opcode != IROpcode::NOP) {
                        ...
                        // Propagate float type: if either operand is float, result is float
                        IRType lt = typeOfRef(lRef), rt = typeOfRef(rRef);
                        IRType resType = (lt == IRType::FLOAT || rt == IRType::FLOAT)
    ...
    void gen(const ASTNode& n) {
        ...
        case NodeType::MethodCall: {
            ...
            if (!n.children.empty() && (endsWith(mname,"display") || endsWith(mname,"print") ||
                                        endsWith(mname,"log") || mname == "alert" || mname == "sure")) {
        ...
        case NodeType::TryCatchStmt: {
            ...
            // value = exception variable name (from 'report <var>')
            emit(IRInstruction(IROpcode::TRY_BEGIN));

            if (n.children.size() > 0) gen(*n.children[0]); // try body

            {
                IRInstruction cb(IROpcode::CATCH_BEGIN);
                cb.typedOperands = {mkConst(n.value)}; // exception var name
                emit(std::move(cb));
            }

            if (n.children.size() > 1) gen(*n.children[1]); // catch body

            if (n.children.size() > 2) {
                emit(IRInstruction(IROpcode::AFTER_BEGIN));
                gen(*n.children[2]); // after body
            }
