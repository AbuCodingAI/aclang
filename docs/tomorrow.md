...
- `MemInt` (arbitrary precision; allocates until safeguards / target size)

MemInt safeguard — **DECIDED**: default cap **64 MB per value** (≈150 million decimal digits).
- Rationale: 52! ≈ 68 digits; ~20,000,000! would be needed to reach 150M digits — astronomically
  past any real use, yet 64 MB is *tiny* as memory. Graham's number is NOT storable positionally
  at any cap (only symbolically), so MemInt deliberately does not chase that.
- Behavior: check the **projected** size *before* each grow; exceeding the cap raises a clean,
  catchable AC error (`Preposterous: MemInt exceeded its 64 MB limit`) — never let `malloc` fail
  or the OOM-killer fire. Fail safe, not catastrophic. (Matches AC's guardrail philosophy:
  self-imposed limits, like the anti-DoS ping rate-limit.)
- Configurable ceiling (`math.MemInt` settable max) so record-computation can opt into a higher
  cap *deliberately* — but it never defaults to unbounded.
- Consider an aggregate budget later (per-value caps don't stop N small MemInts from OOMing).
- Display: huge values should default to digit-count + scientific form ("show all" on request)
  so printing a MemInt isn't its own DoS (the "googol-book" problem).
- Bake the cap in from the FIRST commit — a bignum type that ships "unbounded" reaches prod first.
