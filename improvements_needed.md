...
Current library reality: most ilibs are real, not stubs. `math`, `os`, `regex`,
`gl`, `widgets`, `machine-audio`, `pointers`, `string-cheese`, `aczip`, and
`web` have real implementation work behind them. `web` now has compiler
scaffolding for compiled backends. `ml` remains the true placeholder/spec-first
library.
...
Keep status documents synced with code. `MATURITY.md`, `BNY_PHASE_STATUS.md`,
and this file drifted stale and understated implemented work, especially BNY
arrays and ilib status.
...
~~change file extension from .aca(AC Asterisk) to .acs(AC Star)~~ ✅ DONE — dir `AC*`→`ACS`, token `aca`→`acs`, ext `.acs`, binary `acs`. Language display name stays "AC*".
