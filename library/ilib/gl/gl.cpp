...
// Extract the first integer anywhere in a spec ("screen - 80" → 80, "80" → 80).
static int _spec_num(const char* s) {
    while (*s && (*s < '0' || *s > '9')) s++;
    return *s ? atoi(s) : 0;
}
static bool _has(const char* s, const char* kw) { return strstr(s, kw) != nullptr; }

void ac_gl_obj_pos_from_spec(const char* name, const char* x_spec, const char* y_spec) {
    int x = 0, y = 0;
    // "screen" anywhere (with or without spaces around the '-') = measured from far edge.
    if (_has(x_spec, "center"))      x = (_scr_w - ac_gl_obj_w(name)) / 2;
    else if (_has(x_spec, "right"))  x = _scr_w - ac_gl_obj_w(name);
    else if (_has(x_spec, "left"))   x = 0;
    else if (_has(x_spec, "screen")) x = _scr_w - _spec_num(x_spec);
    else x = atoi(x_spec);

    if (_has(y_spec, "center"))       y = (_scr_h - ac_gl_obj_h(name)) / 2;
    else if (_has(y_spec, "bottom"))  y = _scr_h - ac_gl_obj_h(name);
    else if (_has(y_spec, "top"))     y = 0;
    else if (_has(y_spec, "screen"))  y = _scr_h - _spec_num(y_spec);
    else y = atoi(y_spec);
