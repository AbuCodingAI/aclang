...
def web_page_get(url):
    """Fetch page contents as a string via HTTP GET. Adds https:// if no scheme.
...
# Namespace object so AC's `web.page_get(...)` calls resolve (like maudio.*).
class web:
    open      = staticmethod(web_open)
    file_open = staticmethod(web_file_open)
    popen     = staticmethod(web_popen)
    ropen     = staticmethod(web_ropen)
    browser   = staticmethod(web_browser)
    pdf       = staticmethod(web_pdf)
    text      = staticmethod(web_text)
    inspect   = staticmethod(web_inspect)
    ac_page   = staticmethod(web_ac_page)
    page_get  = staticmethod(web_page_get)
    help      = staticmethod(web_help)
