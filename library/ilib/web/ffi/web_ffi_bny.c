...
typedef const char* (*web_page_get_t)(const char*);
...
const char* web_page_get(const char* url) {
    load_library();
    web_page_get_t func = (web_page_get_t)dlsym(lib_handle, "ac_web_page_get");
    if (!func) {
        fprintf(stderr, "Error: ac_web_page_get not found\n");
        return "";
    }
    return func(url);
}
