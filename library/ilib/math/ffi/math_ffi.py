...
_acmath_lib = _os.path.join(globals().get('_ac_math_lib_dir', _os.path.join(_os.getcwd(), 'library', 'ilib', 'math')), 'acmath.dll' if _pl.system() == 'Windows' else 'libacmath.so')
...
math_pow=_d2('ac_pow'); math_hypot=_d2('ac_hypot')
_math_mod_raw=_d2('ac_mod')
def math_mod(a, b):  # int-exact when operands and result are whole (C ac_mod returns double)
    r = _math_mod_raw(a, b)
    ri = int(r)
    return ri if ri == r else r
...
math_phi=_c0('ac_math_phi_const')()
...
_m.ac_math_phi.argtypes=[_INT]; _m.ac_math_phi.restype=_D
math_pi_digits=lambda n: _m.ac_math_pi(int(n)); math_e_digits=lambda n: _m.ac_math_e(int(n))
math_phi_digits=lambda n: _m.ac_math_phi(int(n))
...
_PHI_100 = "1.6180339887498948482045868343656381177203091798057628621354486227052604628189024497072072041893911374"
...
def _phi_prec(n):
    n = int(n)
    if n <= 0:  return _PHI_100[:1]
    if n <= 15: return str(_m.ac_math_phi(n))
    end = 2 + n
    return _PHI_100[:min(end, len(_PHI_100))]
...
phi=_PrecFloat(math_phi, _phi_prec)
