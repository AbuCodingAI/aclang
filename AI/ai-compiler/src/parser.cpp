...
private:
    ...
    NodePtr parsePrefix() {
        ...
        if (at(TokenType::IDENTIFIER)) {
            ...
            // No-paren type conversions: to_dec x, to_dec $5$, to_int y, etc.
            // Also accepts parens: to_dec(0.1)
            if (tok.value == "to_dec" || tok.value == "to_int" ||
                tok.value == "to_string" || tok.value == "to_bool") {
...
private:
    ...
    NodePtr parseIdentifierStatement() {
        ...
        if (at(TokenType::DOT)) {
            ...
            // All methods: use Pratt parser for arguments so children are proper AST nodes.
            // Supports: Name.method(args), Name.method $str$, Name.method expr
            {
                auto node = std::make_unique<ASTNode>(NodeType::MethodCall, name + "." + prop);
                if (at(TokenType::LPAREN)) {
                    advance();
                    ...
                    if (at(TokenType::RPAREN)) advance();
                           ...
                           !at(TokenType::RBRACKET) && !at(TokenType::RBRACE)) {
                    auto argExpr = parseExpression(0);
                    if (argExpr) node->children.push_back(std::move(argExpr));
                }
                return node;
        ...
        if (at(TokenType::ASSIGN)) {
            ...
            // Retype cast: x = dec expr / x = int expr / x = string expr / x = bool expr
            // dec/int/string/bool as a cast-operator, not a type declaration
            if (at(TokenType::KW_DEC) || at(TokenType::KW_INT) ||
                at(TokenType::KW_STRING) || at(TokenType::KW_BOOL)) {
                std::string coerceType;
                if      (at(TokenType::KW_DEC))    coerceType = "DEC";
                else if (at(TokenType::KW_INT))    coerceType = "INT";
                else if (at(TokenType::KW_STRING)) coerceType = "STRING";
                else                               coerceType = "BOOL";
                advance(); // consume keyword
                auto valExpr = parseExpression(0);
                auto coerceNode = std::make_unique<ASTNode>(NodeType::TypeCoerceStmt, name);
                coerceNode->attrs.push_back(coerceType);
                if (valExpr) coerceNode->children.push_back(std::move(valExpr));
                return coerceNode;
            }
