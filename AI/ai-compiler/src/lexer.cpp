...
public:
    ...
    std::vector<Token> tokenize() {
        ...
        while (pos < src.size()) {
            ...
            // String: $...$ (escape sequences) and r$...$ (raw, no escapes)
            if (src[pos] == '$') {
                bool rawStr = !tokens.empty() &&
                              tokens.back().type == TokenType::IDENTIFIER &&
                              tokens.back().value == "r";
                if (rawStr) tokens.pop_back();
            ...
            // Backend: AC->XX, AI->XX, or AC LIB (library header)
            if (src.substr(pos, 3) == "AC-" || src.substr(pos, 3) == "AI-") {
                int sc = col;
                pos += 3; col += 3;
