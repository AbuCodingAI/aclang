...
static const std::unordered_map<std::string, TokenType> KEYWORDS = {
    ...
    {"quickthread", TokenType::KW_QUICKTHREAD},
    ...
    {"compfold", TokenType::KW_COMPFOLD},
    {"cp",       TokenType::KW_CP},
...
public:
    ...
    int line = 1;
    int col = 1;
    bool warnedSemicolon = false; // roast C-style ';' once per file
    ...
    std::vector<Token> tokenize() {
        ...
        while (pos < src.size()) {
            ...
            if (src[pos] == '\n') {
                ...
                if (spaces > current) {
                    ...
                    // #34: dedent must land on an EXISTING level (4→8→6 was silently accepted
                    // and re-parented statements to the wrong block).
                    if (spaces != indentStack.back()) {
                        std::cerr << "Preposterous: IndentError: line " << line
                                  << " dedents to " << spaces
                                  << " spaces, which matches no enclosing block (expected "
                                  << indentStack.back() << ")\n";
                        std::exit(1);
                    }
                }
            ...
            // Tag: <tagname> — only if < is followed by a letter AND a '>' closes it on the
            ...
            if (src[pos] == '/') {
                ...
                // Check for /// (always-float division), then // (floor division), before /=
                if (pos + 1 < src.size() && src[pos] == '/' && src[pos+1] == '/') {
                    pos += 2; col += 2;
                    tokens.emplace_back(TokenType::TRIPLE_SLASH, "///", line, sc);
            ...
            if (std::isalpha(src[pos]) || src[pos] == '_') {
                ...
                // Ruby-style trailing '?' / '!' are part of the identifier: valid?, save!, empty?
                // They aren't legal names in the target languages, so mangle to a portable form
                // ('?'→"_q", '!'→"_bang") — deterministic, so every use maps to the same symbol.
                while (pos < src.size() && (src[pos] == '?' || src[pos] == '!')) {
                    word += (src[pos] == '?') ? "_q" : "_bang";
                    pos++; col++;
                }
                ...
                if (pos < src.size() && src[pos] == '-' && pos + 1 < src.size() &&
                    ...
                    std::string ext = "-";
                    size_t p = pos + 1; int c = col + 1;
                    size_t bestP = 0; int bestC = 0; std::string bestExt;
                    while (p < src.size() && (std::isalnum((unsigned char)src[p]) || src[p] == '_' || src[p] == '-')) {
                        ext += src[p++]; c++;
                        if (KEYWORDS.find(word + ext) != KEYWORDS.end()) { bestP = p; bestC = c; bestExt = ext; }
                    }
                    if (bestP) { word += bestExt; pos = bestP; col = bestC; }
                }
        ...
        // Emit remaining DEDENTs so a file ending inside an indented block (no trailing
        // newline) doesn't throw "Expected dedent" on otherwise-valid code.
        while (indentStack.size() > 1) { indentStack.pop_back(); tokens.emplace_back(TokenType::DEDENT, "", line, col); }
