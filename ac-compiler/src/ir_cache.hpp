...
static void wInstr(std::ofstream& f, const AC_IR::IRInstruction& ins) {
    ...
    // attrs carry semantic markers ("copy", "bound") — losing them on cache reload
    // silently changed semantics on the 2nd compile.
    uint8_t na = (uint8_t)std::min((size_t)255, ins.attrs.size());
    wU8(f, na);
    for (size_t i = 0; i < na; i++) wStr(f, ins.attrs[i]);
    wU8(f, (uint8_t)ins.resultType);  // #9: TYPE_CAST's target type lives ONLY here — dropping it
                                      // made every int/dec/string cast VOID on a cached 2nd compile
}
...
static AC_IR::IRInstruction rInstr(std::ifstream& f) {
    ...
    auto ins = AC_IR::IRInstruction(op, res, std::move(ops));
    uint8_t na = rU8(f);
    for (int i = 0; i < na; i++) ins.attrs.push_back(rStr(f));
    ins.resultType = (AC_IR::IRType)rU8(f);   // #9
    return ins;
}
...
inline void saveIRCache(const std::string& ircFile, uint64_t hash,
    ...
    wU8(f, prog.infiniteMainloop ? 1 : 0);
...
inline std::unique_ptr<AC_IR::IRProgram> loadIRCache(const std::string& ircFile,
    ...
    for (uint32_t i = 0; i < sc; i++) {
        ...
        auto type  = (AC_IR::IRType)rU8(f);
        int scope  = rI32(f);
        // Must preserve IDs 1:1 — instructions reference symbols by ID. intern() would
        // dedup same-name symbols across scopes and shift every later ID.
        prog->symbols.restoreSymbol(name, type, scope);
    }
