...
private:
    ...
    int getPrecedence(TokenType op) {
        switch (op) {
            ...
            // (#1: += / -= are STATEMENTS — giving them infix precedence with no parseInfix
            // case made the Pratt loop spin forever on `Term.display total += 1`.)
    ...
    NodePtr parsePrefix() {
        ...
        if ((at(TokenType::KW_STRING) || at(TokenType::KW_INT) ||
            ...
            auto node = std::make_unique<ASTNode>(NodeType::UnaryExpr, kind);
            if (inner) node->children.push_back(std::move(inner));
            return node;
        }

        // Unary ~ / bnot (bitwise NOT)
        if (at(TokenType::TILDE) || at(TokenType::KW_BNOT)) {
            advance();
            auto operand = parseExpression(9); // unary binds tighter than +/-/* (grabs only its operand + ^)
        ...
        if (at(TokenType::IDENTIFIER) || at(TokenType::KW_VALUE) || at(TokenType::KW_RULE)) {
            ...
            if (at(TokenType::LBRACKET)) {
                ...
                // Indexed call: funcs[i](args) — the element is the callee.
                // CallExpr with empty value marks an INDIRECT call; children[0] = callee expr.
                if (at(TokenType::LPAREN)) {
                    advance();
                    ...
                    expect(TokenType::RPAREN, "Expected ')' after indexed-call arguments");
                    return call;
                }
                return node;
            }
        ...
        // sequence(a, b[, step]) — list; stream(a, b[, step]) — lazy, concatenated display
        if (at(TokenType::KW_SEQUENCE) || at(TokenType::KW_STREAM)) {
            bool isStream = at(TokenType::KW_STREAM);
            advance();
            expect(TokenType::LPAREN, "Expected ( after sequence/stream");
            auto xExpr = parseExpression(0);
            expect(TokenType::COMMA, "Expected , in sequence/stream(a, b)");
            auto yExpr = parseExpression(0);
            NodePtr stepExpr;
            if (at(TokenType::COMMA)) { advance(); stepExpr = parseExpression(0); }
            expect(TokenType::RPAREN, "Expected ) after sequence/stream");
            auto node = std::make_unique<ASTNode>(
                isStream ? NodeType::StreamExpr : NodeType::SequenceExpr, "");
            if (xExpr) node->children.push_back(std::move(xExpr));
            if (yExpr) node->children.push_back(std::move(yExpr));
            if (stepExpr) node->children.push_back(std::move(stepExpr));
            node->inferredType = std::make_shared<Type>(Type::makeList());
            return node;
        }
        ...
        if (at(TokenType::KW_IOTA)) {
            advance();
            auto boundExpr = parseExpression(0);
            auto node = std::make_unique<ASTNode>(NodeType::IotaExpr, "");
            if (boundExpr) node->children.push_back(std::move(boundExpr));
            node->inferredType = std::make_shared<Type>(Type::makeList());
            return node;
        }
    ...
    NodePtr parseInfix(NodePtr left, TokenType op) {
        ...
        if (op == TokenType::IDENTIFIER) {
            ...
            opStr = "//";
            advance();
        } else if (op == TokenType::TRIPLE_SLASH) {
            opStr = "///";
            advance();
...
private:
    ...
    NodePtr parseStatementInner() {
        ...
        // export — visibility for AC-file imports (use flib x.ac / elib).
        //   Postfix form:  export add[, double, …]   (marks already-defined names)
        //   Prefix form:   export Make f func(…)     (marks the following definition)
        if (at(TokenType::KW_EXPORT)) {
            advance(); // consume 'export'
            if (at(TokenType::IDENTIFIER)) {
                auto node = std::make_unique<ASTNode>(NodeType::ExportStmt, "");
        ...
        if (at(TokenType::KW_DESTROY)) {
            advance();
            // destroy a, b, c — collect all names (first = value, rest = attrs)
            auto node = std::make_unique<ASTNode>(NodeType::DestroyStmt, "");
            bool first = true;
            while (at(TokenType::IDENTIFIER) || at(TokenType::KW_VALUE) || at(TokenType::KW_RULE)) {
                std::string nm = advance().value;
                if (first) { node->value = nm; first = false; } else node->attrs.push_back(nm);
                if (at(TokenType::COMMA)) advance(); else break;
            }
            return node;
        }
        ...
        // conglomer <file.h> — dynamically link a native C/C++ header (native backends only).
        // Distinct from `use`: `use` imports AC libraries (ilib/elib/clib/flib); `conglomer`
        // reaches straight into a raw C header + its shared object. Accepts <foo.h>, "foo.h", foo.h.
        if (at(TokenType::KW_CONGLOMER)) {
            advance();
            if (at(TokenType::LT)) advance();           // optional < …
            std::string hdr;
            while (!at(TokenType::NEWLINE) && !at(TokenType::END_OF_FILE) && !at(TokenType::GT)) {
                auto& t = peek();
                if (t.type == TokenType::SLASH)     { advance(); hdr += "/" + t.value; }
                else if (t.type == TokenType::DOT)  { advance(); hdr += "."; }
                else                                  hdr += advance().value;
            }
            if (at(TokenType::GT)) advance();           // optional > close
            auto ns = hdr.find_first_not_of(" \"");
            auto ne = hdr.find_last_not_of(" \"");
            hdr = (ns == std::string::npos) ? "" : hdr.substr(ns, ne - ns + 1);
            return std::make_unique<ASTNode>(NodeType::UseLibStmt, "conglomer:" + hdr);
        }
        ...
        if (at(TokenType::KW_ALIAS)) {
            advance();
            std::string lhs;
            if (at(TokenType::KW_VALUE) || at(TokenType::KW_RULE)) lhs = advance().value;  // #32
            else lhs = expect(TokenType::IDENTIFIER, "Expected variable name after alias").value;
        ...
        // ("value" is a soft keyword — `const value = 2` must work, per LineUp spec)
        if (at(TokenType::KW_CONST)) {
            advance();
            std::string name;
            if (at(TokenType::KW_VALUE) || at(TokenType::KW_RULE)) name = advance().value;
            else name = expect(TokenType::IDENTIFIER, "Expected variable name after const").value;
        ...
        // quickthread f(args) — run the call on a lightweight thread (Go: goroutine;
        // other backends fall back to a plain synchronous call).
        if (at(TokenType::KW_QUICKTHREAD)) {
            advance();
            std::string fname = expect(TokenType::IDENTIFIER, "Expected function name after quickthread").value;
            auto node = std::make_unique<ASTNode>(NodeType::MethodCall, "quickthread");
            node->attrs.push_back(fname);
            if (at(TokenType::LPAREN)) {
                advance();
                ...
                while (!at(TokenType::END_OF_FILE)) {
                    ...
                    if (at(TokenType::RPAREN)) {
                        if (depth > 0) { depth--; cur += advance().value; continue; }
                        advance(); break; // closing paren of the arg list
                    }
                    if (at(TokenType::COMMA) && depth == 0) { advance(); node->attrs.push_back(cur); cur.clear(); continue; }
                    if (at(TokenType::STRING)) cur += "$" + advance().value + "$";
                    else cur += advance().value;
                }
                if (!cur.empty()) node->attrs.push_back(cur);
            }
            return node;
        }

        if (at(TokenType::KW_FREE)) {
            // Distinguish three forms by lookahead after `free`:
            //   Form 2: free var = expr   (also +=, -=, etc.)  → tagged assignment
            //   Form 3: free f(args)                            → tagged call
            //   Form 1: free a, b, c                            → FreeDecl declaration
            if (peekAhead(1).type == TokenType::IDENTIFIER) {
                TokenType after = peekAhead(2).type;
                ...
                if (isAssign) {
                    ...
                    auto assign = parseIdentifierStatement();
                    auto fd = std::make_unique<ASTNode>(NodeType::FreeDecl, "");
                    if (assign) {
                        fd->attrs.push_back(assign->value); // promote target to free scope
                        fd->children.push_back(std::move(assign));
                    }
                    return fd;
                }
            }
        ...
        // bound — persist across the loop, scoped to the ENCLOSING FUNCTION (not global).
        // The right tool for loop accumulators; `free` stays truly global.
        //   bound x = expr / bound x += expr / bound arr[i] = expr  → tagged assignment
        //   bound a, b, c                                           → declaration list
        if (at(TokenType::KW_BOUND)) {
            if (peekAhead(1).type == TokenType::IDENTIFIER) {
                TokenType after = peekAhead(2).type;
                bool declList = (after == TokenType::COMMA || after == TokenType::NEWLINE
                              || after == TokenType::END_OF_FILE);
                if (!declList) {
                    advance(); // consume 'bound'
                    auto assign = parseIdentifierStatement();
                    auto bd = std::make_unique<ASTNode>(NodeType::FreeDecl, "bound");
                    if (assign) {
                        bd->attrs.push_back(assign->value);
                        bd->children.push_back(std::move(assign));
                    }
                    return bd;
                }
            }
            advance(); // consume 'bound'
            auto node = std::make_unique<ASTNode>(NodeType::FreeDecl, "bound");
            while (!at(TokenType::NEWLINE) && !at(TokenType::END_OF_FILE)) {
                if (at(TokenType::IDENTIFIER)) node->attrs.push_back(advance().value);
    ...
    NodePtr parseTagBlock() {
        ...
        if (closed) {
            ...
            if (at(TokenType::TAG_OPEN) && peek().value == closeWith) {
                ...
                // No closing tag. For <mainloop> this is the "infinite loop" form — mark it so
                // IR wraps the body in while(true); the compiler then requires --allow-infinite.
                if (name == "mainloop") block->attrs.push_back("__infinite__");
                if (!tagStack.empty() && tagStack.back() == name) tagStack.pop_back();
            }
        }
    ...
    // #18: DEPTH-AWARE — nested [[1,2],[3,4]] used to truncate at the first ']'.
    NodePtr parseListLiteral() {
        advance(); // consume [
        std::string contents;
        int depth = 0;
        while (!at(TokenType::END_OF_FILE)) {
            if (at(TokenType::RBRACKET) && depth == 0) break;
            if (at(TokenType::LBRACKET))      { depth++; contents += "["; advance(); continue; }
            if (at(TokenType::RBRACKET))      { depth--; contents += "]"; advance(); continue; }
            if (at(TokenType::STRING))        { contents += "$" + advance().value + "$"; continue; }
            if (at(TokenType::COMMA))         { contents += ","; advance(); continue; }
            contents += advance().value;
        }
    ...
    NodePtr parseMake() {
        ...
        std::string name;
        if (at(TokenType::KW_VALUE) || at(TokenType::KW_RULE)) name = advance().value;  // #32
        else name = expect(TokenType::IDENTIFIER, "Expected name after Make").value;
    ...
    NodePtr parseIdentifierStatement() {
        ...
        if (at(TokenType::ASSIGN)) {
            advance();
            // list literal — but [elem] @ n is LIST REPETITION, an expression: keep parsing
            if (at(TokenType::LBRACKET)) {
                auto listNode = parseListLiteral();
                if (at(TokenType::AT) || at(TokenType::MULTIPLY)) {
                    advance(); // consume @ (returning early here silently dropped "@ n")
                    auto count = parseExpression(0);
                    auto rep = std::make_unique<ASTNode>(NodeType::BinaryExpr, "@");
                    rep->children.push_back(std::move(listNode));
                    rep->children.push_back(std::move(count));
                    auto node = std::make_unique<ASTNode>(NodeType::AssignStmt, name);
                    node->children.push_back(std::move(rep));
                    return node;
                }
                auto node = std::make_unique<ASTNode>(NodeType::AssignStmt, name);
