...
class CppCodeGen {
    ...
    void genNode(const ASTNode& node) {
        ...
        case NodeType::Program: {
            ...
            // Scan for use ilib statements and emit their includes first
            std::function<void(const ASTNode&)> emitLibIncludes = [&](const ASTNode& n) {
                ...
                for (auto& c : n.children) emitLibIncludes(*c);
            };
            // Check if any ilib is used
            bool usesLib = false;
            std::function<void(const ASTNode&)> checkLib = [&](const ASTNode& n) {
                if (n.type == NodeType::UseLibStmt) usesLib = true;
                for (auto& c : n.children) checkLib(*c);
            };
            for (auto& c : node.children) checkLib(*c);
            if (usesLib) {
                for (auto& c : node.children) emitLibIncludes(*c);
            }
