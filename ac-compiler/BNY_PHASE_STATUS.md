...
- ✅ INPUT (Term.ask) - stable string input; verified direct display, repeated reads, and string params
...
- `__ac_input_int__()` - integer parser helper
- `__ac_input_str__()` - stable string input helper
...
1. **Array Support Verification**: Arrays are implemented, but need more regression coverage
   - Verified path: allocation, indexing, store, append
   - Required: document the layout and cover nested/mixed cases

2. **Pointer Arithmetic**: Not yet tested
...
- **ARM**: Native BNY machine-code generation is not implemented. The C-backed ARM route is intentional and should be reported as such, not confused with x86-64 X64Emitter output.

## Next Steps (Priority Order)

1. Expand Phase 3 regression tests (arrays, indexing, pointers)
2. Add static linking option for distribution
3. Windows PE support (via ASM backend delegation)
4. Complete Phase 4 (exceptions, eval, events)
