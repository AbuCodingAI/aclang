...
enum class IROpcode {
    ...
    FREE_DECL,  // free x   — declare x as globally scoped (like Python's global)
    TYPE_CAST,  // dec/int/string/bool x — coerce variable to target type (resultType carries target)
    LOAD_INDEX,
    STORE_INDEX,
...
public:
    ...
    // Restore a symbol at the next sequential index WITHOUT deduplication, preserving its
    // original scope. Used by the IR cache loader: symbol IDs are referenced by instructions,
    // so they must round-trip 1:1. intern() would collapse same-name symbols from different
    // scopes into one ID, shifting every later ID and corrupting operand references.
    int restoreSymbol(const std::string& name, IRType type, int scope) {
        int id = static_cast<int>(symbols.size());
        symbols.push_back({name, type, scope});
        nameToIds[name].push_back(id);
        return id;
    }
    ...
    // Update a symbol's type (called when an assignment reveals the type)
    void setType(int id, IRType t) {
        if (id >= 0 && id < static_cast<int>(symbols.size()) && t != IRType::VOID)
            symbols[id].type = t;
    }

    // Get a symbol's IRType (VOID = unknown)
    IRType getType(int id) const {
        if (id >= 0 && id < static_cast<int>(symbols.size()))
